...
Cert_ledger::Cert_ledger(const Config &config, KeyChain &keychain, Face &network, std::shared_ptr<ndn::security::Validator> recordValidator, std::shared_ptr<ndn::security::Validator> eventValidator) :
        m_config(config),
        m_keychain(keychain),
...
void Cert_ledger::onSubscriptionData(const svs::SVSPubSub::SubscriptionData& subData) {
...
ndn::svs::SecurityOptions Cert_ledger::getSecurityOption() {
...
void Cert_ledger::onRecordUpdate(Record record) {
...
bool Cert_ledger::seenEvent(const Name& name) const{
    return m_eventSet.count(name);
}

Cert_ledger::~Cert_ledger() = default;
