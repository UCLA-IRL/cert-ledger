...
void
Mnemosyne::dumpList(const std::map<Name, TailingRecordState> &weight) {
    NDN_LOG_TRACE("Dump " << weight.size() << " Tailing Records");
    for (const auto &item : weight) {
        NDN_LOG_TRACE((item.second.parentEndorseVerified ? "OK " : "NO ") << item.second.refSet.size() << "\t"
                                                                          << item.first.toUri());
    }
}
...
Mnemosyne::Mnemosyne(const Config &config,
    ...
    m_network.setInterestFilter(m_config.peerPrefix, bind(&Mnemosyne::onRecordRequest, this, _2), nullptr, nullptr);
    m_network.setInterestFilter(syncName, bind(&Mnemosyne::onLedgerSyncRequest, this, _2), nullptr, nullptr);
...
Mnemosyne::createRecord(Record &record) {
    ...
    if (record.getType() == CERTIFICATE_RECORD) {
        for (const auto &certName: m_lastCertRecords) {
            NDN_LOG_INFO("[Mnemosyne::addRecord] Certificate record: Add previous cert record: " << certName);
...
Mnemosyne::listRecord(const std::string &prefix) const {
    ...
    list.remove_if([&](const auto &name) {
        return m_tailRecords.count(name) && !m_tailRecords.find(name)->second.parentEndorseVerified;
    });
    return list;
}

optional<Record>
Mnemosyne::getRecord(const Name &rName) const {
...
bool
Mnemosyne::seenRecord(const Name &recordName) const {
    if (m_tailRecords.count(recordName)) {
        return true;
    }
    auto dataPtr = m_backend.getRecord(Name(recordName));
...
void
Mnemosyne::onNack(const Interest &, const lp::Nack &nack) {
    NDN_LOG_ERROR("Received Nack with reason " << nack.getReason());
}

void
Mnemosyne::onTimeout(const Interest &interest) {
    NDN_LOG_ERROR("Timeout for " << interest);
}

ReturnCode
Mnemosyne::sendSyncInterest() {
    NDN_LOG_INFO("[Mnemosyne::sendSyncInterest] Send SYNC Interest.");
    ...
    m_network.expressInterest(syncInterest, nullptr,
                              bind(&Mnemosyne::onNack, this, _1, _2), nullptr);
...
bool
Mnemosyne::checkSyntaxValidityOfRecord(const Data &data) {
    NDN_LOG_INFO("[Mnemosyne::checkSyntaxValidityOfRecord] Check the format validity of the record");
    ...
    try {
        ...
        NDN_LOG_ERROR("[Mnemosyne::checkSyntaxValidityOfRecord] The Data format is not proper for DLedger record "
                              << dataRecord.getRecordName() << " because " << e.what());
        return false;
    }
    ...
    if (!m_config.certificateManager->verifySignature(data)) {
        NDN_LOG_ERROR("[Mnemosyne::checkSyntaxValidityOfRecord] Bad Signature for " << dataRecord.getRecordName());
        return false;
    }
    ...
    if (tp > time::system_clock::now() + m_config.clockSkewTolerance) {
        NDN_LOG_ERROR("[Mnemosyne::checkSyntaxValidityOfRecord] record from too far in the future"
                              << dataRecord.getRecordName());
        return false;
    }
    ...
    for (const auto &precedingRecordName : dataRecord.getPointersFromHeader()) {
        ...
        if (RecordName(precedingRecordName).getProducerPrefix() == producerID) {
            NDN_LOG_ERROR("[Mnemosyne::checkSyntaxValidityOfRecord] Preceding record From itself: "
                                  << dataRecord.getRecordName());
            return false;
        }
    }
    ...
    if (dataRecord.getType() == CERTIFICATE_RECORD || dataRecord.getType() == REVOCATION_RECORD) {
        if (!m_config.certificateManager->verifyRecordFormat(dataRecord)) {
            NDN_LOG_ERROR("[Mnemosyne::checkSyntaxValidityOfRecord] bad certificate/revocation record: "
                                  << dataRecord.getRecordName());
            return false;
        }
    } else {
...
bool
Mnemosyne::checkEndorseValidityOfRecord(const Data &data) {
    NDN_LOG_INFO("[Mnemosyne::checkEndorseValidityOfRecord] Check the reference validity of the record");
    Record dataRecord;
    try {
        // format check
        dataRecord = Record(data);
    ...
    if (!m_config.certificateManager->endorseSignature(data)) {
        NDN_LOG_INFO(
                "[Mnemosyne::checkEndorseValidityOfRecord] certificate revoked for " << dataRecord.getRecordName());
        return false;
    }
    ...
    for (const auto &precedingRecordName : dataRecord.getPointersFromHeader()) {
        if (m_tailRecords.count(precedingRecordName) != 0) {
            NDN_LOG_TRACE("-- Preceding record " << precedingRecordName << " has weight "
                                                 << m_tailRecords[precedingRecordName].refSet.size());
            if (m_tailRecords[precedingRecordName].refSet.size() > m_config.contributionWeight) {
                NDN_LOG_WARN(
                        "[Mnemosyne::checkEndorseValidityOfRecord] Weight too high for " << dataRecord.getRecordName()
                                                                                         << " with weight "
                                                                                         << m_tailRecords[precedingRecordName].refSet.size());
                return false;
            }
            ...
            if (m_backend.getRecord(precedingRecordName) != nullptr) {
                NDN_LOG_WARN("[Mnemosyne::checkEndorseValidityOfRecord] Preceding record " << precedingRecordName
                                                                                           << " too deep");
            } else {
                NDN_LOG_WARN("[Mnemosyne::checkEndorseValidityOfRecord] Preceding record " << precedingRecordName
                                                                                           << " Not found");
...
void
Mnemosyne::onLedgerSyncRequest(const Interest &interest) {
    // verify the signature
    if (!m_config.certificateManager->verifySignature(interest)) {
        NDN_LOG_ERROR("[Mnemosyne::onLedgerSyncRequest] Bad SYNC Signature: " << interest.getName());
        return;
    }
    NDN_LOG_INFO("[Mnemosyne::onLedgerSyncRequest] Receive SYNC Interest");
        ...
        if (item.type() == tlv::KeyLocator) {
            try {
                ...
                if (!seenRecord(certName)) {
                    NDN_LOG_INFO("[Mnemosyne::onLedgerSyncRequest] Fetch unseen certificate record " << l.getName());
                    fetchRecord(certName);
                    isCertPending = true;
                }
            } catch (const std::exception &e) {
                NDN_LOG_ERROR("[Mnemosyne::onLedgerSyncRequest] Error on keyLocator");
            }
            continue;
        }
    ...
    if (shouldSendSync) {
        NDN_LOG_INFO("[Mnemosyne::onLedgerSyncRequest] send Sync interest so others can fetch new record");
...
void
Mnemosyne::onRecordRequest(const Interest &interest) {
    auto desiredData = getRecord(interest.getName());
    if (desiredData) {
        NDN_LOG_INFO("[Mnemosyne::onRecordRequest] Reply Data: " << interest.getName());
        m_network.put(*desiredData->m_data);
    } else {
        NDN_LOG_ERROR("[Mnemosyne::onRecordRequest] Data not Found: " << interest.getName());
    }
}

void
Mnemosyne::fetchRecord(const Name &recordName) {
    Interest interestForRecord(recordName);
    interestForRecord.setCanBePrefix(false);
    interestForRecord.setMustBeFresh(true);
    NDN_LOG_INFO("[Mnemosyne::fetchRecord] Fetch the record: " << interestForRecord.getName().toUri());
    m_network.expressInterest(interestForRecord,
                              bind(&Mnemosyne::onFetchedRecord, this, _1, _2),
                              bind(&Mnemosyne::onNack, this, _1, _2),
                              bind(&Mnemosyne::onTimeout, this, _1));
}

void
Mnemosyne::onFetchedRecord(const Interest &interest, const Data &data) {
    if (seenRecord(data.getFullName())) {
        NDN_LOG_INFO("[Mnemosyne::onFetchedRecord] Record already exists in the ledger. Ignore " << data.getFullName());
        return;
    }
    for (const auto &stackRecord : m_syncStack) {
        if (stackRecord.first.getRecordName() == data.getFullName()) {
            NDN_LOG_INFO("[Mnemosyne::onFetchedRecord] Record in sync stack already. Ignore " << data.getFullName());
            return;
        }
    }
    NDN_LOG_INFO("[Mnemosyne::onFetchedRecord] fetched new record " << data.getFullName());
    ...
    try {
        ...
        NDN_LOG_ERROR("- The Data format of " << data.getFullName() << " is not proper for DLedger record because "
                                              << e.what());
        return;
    }
    ...
    while (stackSize != m_syncStack.size()) {
        ...
        NDN_LOG_INFO("[Mnemosyne::onFetchedRecord] SyncStack size " << m_syncStack.size());
        for (auto it = m_syncStack.begin(); it != m_syncStack.end();) {
            if (checkRecordAncestor(it->first)) {
                ...
                NDN_LOG_WARN("[Mnemosyne::onFetchedRecord] Timeout on fetching ancestor for "
                                     << it->first.getRecordName().toUri());
                it = m_syncStack.erase(it);
            } else {
...
bool
Mnemosyne::checkRecordAncestor(const Record &record) {
...
void
Mnemosyne::addToTailingRecord(const Record &record, bool endorseVerified) {
    if (m_tailRecords.count(record.getRecordName()) != 0) {
        NDN_LOG_INFO("[Mnemosyne::addToTailingRecord] Repeated add record: " << record.getRecordName());
        return;
    }
    NDN_LOG_TRACE("[Mnemosyne::addToTailingRecord] adding record" << record.getRecordName());
    ...
    while (!stack.empty()) {
        ...
        for (const auto &precedingRecord : precedingRecordList) {
            ...
            if (m_tailRecords.count(precedingRecord) != 0 &&
                ...
                NDN_LOG_TRACE("[Mnemosyne::addToTailingRecord]" << record.getProducerPrefix() << " confirms "
                                                                << precedingRecord.toUri());
            }
        }
    }
    ...
    for (const auto &updatedRecord : updatedRecords) {
        ...
        if (tailingState.refSet.size() == m_config.confirmWeight) {
            NDN_LOG_INFO("[Mnemosyne::addToTailingRecord]" << updatedRecord.toUri() << " is confirmed");
...
void
Mnemosyne::onRecordConfirmed(const Record &record) {
    NDN_LOG_INFO("[Mnemosyne::onRecordConfirmed] accept record" << record.getRecordName());
    ...
    if (record.getType() == RecordType::CERTIFICATE_RECORD) {
        try {
            ...
            NDN_LOG_ERROR(
                    "[Mnemosyne::onRecordConfirmed] Bad certificate record format for " << record.getRecordName());
            return;
        }
    }
...
void
Mnemosyne::removeTimeoutRecords() {
    ...
    while (!timeoutList.empty()) {
        for (const auto &i : timeoutList) {
            ...
            NDN_LOG_INFO("[Mnemosyne::removeTimeoutRecords] remove timeout record " << i);
        }
